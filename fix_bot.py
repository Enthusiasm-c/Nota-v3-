#!/usr/bin/env python3
# Скрипт для замены проблемной функции в файле bot.py
...
# Исправленная версия функции handle_field_edit с правильными отступами
fixed_function = '''@with_async_retry_backoff(max_retries=2, initial_backoff=1.0, backoff_factor=2.0)
...
# Ищем начало и конец функции handle_field_edit
pattern = r'@with_async_retry_backoff.*?async def handle_field_edit\(.*?await state\.set_state\(NotaStates\.editing\)[^\n]*'
replaced_content = re.sub(pattern, fixed_function, content, flags=re.DOTALL)

# Записываем исправленное содержимое обратно в файл
with open(f'{bot_file}.fixed', 'w') as f:
    f.write(replaced_content)

print(f"Создан исправленный файл {bot_file}.fixed")
print("Проверьте его и переименуйте в bot.py для применения изменений.") 