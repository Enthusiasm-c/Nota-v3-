...
if [ -d "${PROJECT_DIR}/venv" ]; then
    ...
    echo -e "${GREEN}[+] Обнаружено виртуальное окружение (.venv), активирую...${NC}"
    source "${PROJECT_DIR}/.venv/bin/activate"
    PYTHON="${PROJECT_DIR}/.venv/bin/python"
else
...
if [ -f "${PROJECT_DIR}/.env" ]; then
    echo -e "${GREEN}[+] Загружаю переменные окружения из .env...${NC}"
    ...
    # Выводим загруженные ключи (без значений для безопасности)
    echo -e "${GREEN}[+] Загружены следующие переменные окружения:${NC}"
    grep -v '^#' "${PROJECT_DIR}/.env" | cut -d= -f1 | while read -r key; do
        echo -e "  - ${key}"
    done
else
    echo -e "${RED}[-] Файл .env не найден, продолжаю без переменных окружения${NC}"
fi
...
"${PYTHON}" -X dev "${PROJECT_DIR}/bot.py" 2>&1 | tee -a "${LOG_FILE}" "${ERROR_LOG}" 