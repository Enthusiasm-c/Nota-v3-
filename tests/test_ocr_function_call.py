...
async def test_openai_function_call(monkeypatch):
    ...
    message.content = [
        type(
            ...
            {
                "type": "text",
                "text": '{"supplier": "Test Supplier", "positions": [{"name": "Tuna loin", "qty": 1, "unit": "kg", "price": 100.0}]}',
            },
        )()
    ]
    messages = AsyncMock()
    ...
    with vcr.use_cassette(CASSETTE_PATH):
        ...
        assert (
            parsed.date is None or isinstance(parsed.date, str) or hasattr(parsed.date, "isoformat")
        )
