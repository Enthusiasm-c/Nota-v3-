...
@pytest.mark.parametrize("user_input,invoice_lines,expected", [
    ...
    ("общая сумма 12,34,56", None, [{"action": "unknown", "error": "unknown_command", "original_text": "общая сумма 12,34,56", "source": "integrated_parser"}]), # free_parser might handle this
    # Неоднозначные команды или команды с опечатками
    ("поставщиик ООО Ромашка", None, [{"action": "unknown", "error": "unknown_command", "original_text": "поставщиик ООО Ромашка", "source": "integrated_parser"}]), 
    ("totall 1234", None, [{"action": "unknown", "error": "unknown_command", "original_text": "totall 1234", "source": "integrated_parser"}]), 
    ("change name in row two to Bread", None, [{"action": "unknown", "error": "unknown_command", "original_text": "change name in row two to Bread", "source": "integrated_parser"}]),
    # Многострочные команды
    ("поставщик ООО Ромашка\nстрока 1 количество 2", None, [
        {"action": "unknown", "error": "unknown_command", "original_text": "поставщик ООО Ромашка", "source": "integrated_parser"}, # Free parser handles supplier
        {"action": "edit_quantity", "line": 0, "value": 2.0, "source": "local_parser"}
    ]),
    ("supplier Acme Corp; row 2 name Milk", None, [
        {"action": "unknown", "error": "unknown_command", "original_text": "supplier Acme Corp", "source": "integrated_parser"}, # Free parser handles supplier
    ...
    ("line 3: name Cream Cheese; price 250; qty 15; unit krat", None, [
        {"action": "edit_name", "line": 2, "value": "Cream Cheese", "source": "local_parser"},
...
def test_parse_commands_edge_cases(user_input, invoice_lines, expected):
    result = parse_compound_command(user_input, invoice_lines) if invoice_lines is not None else parse_compound_command(user_input)
