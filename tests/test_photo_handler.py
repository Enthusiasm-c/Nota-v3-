...
async def test_photo_ok(monkeypatch, fake_msg):
    ...
    mock_bot.download_file = AsyncMock(
        return_value=SimpleNamespace(getvalue=lambda: b"img")
    )
    ...
    with patch("bot.create_bot_and_dispatcher", return_value=(mock_bot, mock_dp)), \
        ...
        assert found_report, f"Expected report in answer calls, got: {[c[0][0] if len(c[0])>0 else None for c in answers]}"
...
async def test_photo_error(monkeypatch, fake_msg):
    ...
    with patch("bot.create_bot_and_dispatcher", return_value=(mock_bot, mock_dp)), \
        ...
        found_error = False
        for call in calls:
            if len(call[0]) > 0 and isinstance(call[0][0], str) and "OCR" in call[0][0] and "failed" in call[0][0].lower():
                found_error = True
                break
                
        assert found_error, f"Expected OCR error in answer, got: {[c[0][0] if len(c[0])>0 else None for c in calls]}"
