...
def test_latency_logging_info_and_warning(caplog):
    ...
    with patch.object(assistant_client, "client") as mock_client, patch(
        "app.utils.redis_cache.cache_get", return_value=None
        ...
        with patch("time.time", side_effect=[start, start + 11]):
            ...
            assert any(
                "slow" in r.getMessage().lower() or "warning" in r.getMessage().lower()
                for r in warn_logs
            )
