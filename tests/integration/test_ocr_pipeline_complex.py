...
async def test_vision_only_fallback():
    ...
    sample_vision_result = json.dumps(
        {
            "lines": [
                ...
                {"name": "HDMI Adapter", "qty": 2, "unit": "pcs", "price": 25.00, "amount": 50.00},
            ]
        }
    )
    ...
    with patch("app.ocr_pipeline.get_detector", return_value=failing_detector), patch(
        "app.ocr_pipeline.call_openai_ocr_async", new_callable=AsyncMock
    ) as mock_vision_api, patch("app.ocr_pipeline.settings") as mock_settings:
