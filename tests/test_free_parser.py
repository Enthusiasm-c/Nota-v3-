...
# Tests for Step 1: Verify error intents for invalid numeric values
def test_detect_intent_invalid_numeric_qty_ru():
    intent = free_parser.detect_intent("строка 1 количество текст")
    expected = {
        "action": "unknown", 
...
def test_detect_intent_invalid_numeric_price_ru():
    intent = free_parser.detect_intent("строка 2 цена многоденег")
    expected = {
        "action": "unknown", 
        "error": "invalid_numeric_value", 
        "field": "price", 
        "original_value": "многоденег", 
        "source": "free_parser"
    }
    assert intent == expected

def test_detect_intent_invalid_numeric_qty_en():
    intent = free_parser.detect_intent("line 1 qty abc")
    expected = {
        "action": "unknown", 
        "error": "invalid_numeric_value", 
        "field": "qty", 
...
def test_detect_intent_invalid_numeric_price_en():
    intent = free_parser.detect_intent("line 1 price xyz")
    expected = {
        "action": "unknown", 
        "error": "invalid_numeric_value", 
        "field": "price", 
...
# Tests to ensure valid inputs still work as expected after parser changes
def test_detect_intent_valid_numeric_qty():
    intent = free_parser.detect_intent("строка 1 количество 10.5")
    expected = {"action": "edit_line_field", "line": 1, "field": "qty", "value": 10.5}
    assert intent == expected

def test_detect_intent_valid_numeric_price():
    # Assuming free_parser.py now uses parse_number for price as well
    intent = free_parser.detect_intent("строка 1 цена 100.99")
    expected = {"action": "edit_line_field", "line": 1, "field": "price", "value": 100.99}
    assert intent == expected
    
    intent_comma = free_parser.detect_intent("строка 2 цена 123,45")
    expected_comma = {"action": "edit_line_field", "line": 2, "field": "price", "value": 123.45}
    assert intent_comma == expected_comma

def test_detect_intent_valid_string_name():
    intent = free_parser.detect_intent("строка 1 название новое имя")
    expected = {"action": "edit_line_field", "line": 1, "field": "name", "value": "новое имя"}
    assert intent == expected

def test_detect_intent_valid_string_unit():
    intent = free_parser.detect_intent("строка 1 ед шт.")
    expected = {"action": "edit_line_field", "line": 1, "field": "unit", "value": "шт."} # Assuming .strip() is applied
    assert intent == expected
