...
def mock_table_detector():
    ...
    cells = [
        # Header row
        {"bbox": [50, 200, 300, 230], "image": b"header_image", "structure": {"text": "Product"}},
        {"bbox": [350, 200, 400, 230], "image": b"header_image", "structure": {"text": "Qty"}},
        {"bbox": [450, 200, 500, 230], "image": b"header_image", "structure": {"text": "Unit"}},
        {"bbox": [550, 200, 600, 230], "image": b"header_image", "structure": {"text": "Price"}},
        {"bbox": [650, 200, 700, 230], "image": b"header_image", "structure": {"text": "Total"}},
        # Data row
        {
            "bbox": [50, 250, 300, 280],
            "image": b"data_image",
        ...
        {"bbox": [350, 250, 400, 280], "image": b"data_image", "structure": {"text": "1"}},
        {"bbox": [450, 250, 500, 280], "image": b"data_image", "structure": {"text": "pcs"}},
        {"bbox": [550, 250, 600, 280], "image": b"data_image", "structure": {"text": "100"}},
        {"bbox": [650, 250, 700, 280], "image": b"data_image", "structure": {"text": "100"}},
    ]
...
@pytest.mark.asyncio
async def test_e2e_table_detection_path(
    sample_invoice_png, mock_table_detector, mock_paddle_ocr, mock_validation_pipeline
):
    """Test the full pipeline using table detection path."""
    with patch("app.ocr_pipeline.get_detector", return_value=mock_table_detector), patch(
        "app.ocr_pipeline.PaddleOCR", return_value=mock_paddle_ocr
...
@pytest.mark.asyncio
async def test_e2e_openai_vision_fallback(
    sample_invoice_png, mock_validation_pipeline, mock_openai_vision
):
    ...
    with patch("app.ocr_pipeline.get_detector", return_value=error_detector), patch(
        "app.ocr_pipeline.ValidationPipeline", return_value=mock_validation_pipeline
...
async def test_e2e_complete_failure(sample_invoice_png):
    ...
    with patch("app.ocr_pipeline.get_detector", return_value=error_detector), patch(
        "app.ocr_pipeline.call_openai_ocr", side_effect=Exception("OpenAI API error")
    ):
...
async def test_e2e_invalid_image():
    ...
    with patch("app.ocr_pipeline.get_detector") as mock_get_detector, patch(
        "app.ocr_pipeline.PaddleOCR"
    ) as mock_paddle_ocr, patch("app.ocr_pipeline.ValidationPipeline") as mock_validation:
...
async def test_e2e_no_cells_found(sample_invoice_png):
    ...
    with patch("app.ocr_pipeline.get_detector", return_value=empty_detector), patch(
        "app.ocr_pipeline.ValidationPipeline", return_value=MagicMock()
...
@pytest.mark.asyncio
async def test_parse_numeric_value_in_e2e(
    sample_invoice_png, mock_table_detector, mock_paddle_ocr, mock_validation_pipeline
):
    ...
    with patch("app.ocr_pipeline.get_detector", return_value=mock_table_detector), patch(
        "app.ocr_pipeline.PaddleOCR", return_value=special_paddle_mock
    ), patch("app.ocr_pipeline.ValidationPipeline", return_value=mock_validation_pipeline):
