                ...
                with patch("app.data_loader.load_products") as mock_load:
                    ...
                    with patch("app.formatters.report.build_report") as mock_report:
                        ...
                        if len(positions) >= 3:
                            ...
                            print(
                                f"Единица измерения обновлена до 'pcs': {'✅ Да' if unit_updated else '❌ Нет'}"
                            )
                        ...
                        if (
                            mock_ui.start.called
                            and mock_ui.complete.called
                            and not mock_ui.error.called
                        ):
                ...
                with patch("app.data_loader.load_products"):
                    with patch("app.formatters.report.build_report") as mock_report:
                        ...
                        if len(positions) >= 3:
                            ...
                            print(
                                f"Единица измерения обновлена до 'gram': {'✅ Да' if unit_updated else '❌ Нет'}"
                            )
                            print(f"Текущая единица измерения: {positions[2].get('unit')}")
                        else:
                            print("❌ Не удалось найти позицию 3")
