...
async def test_edit_message_text_safe_success():
    ...
    await clear_edit_cache()
    await edit_message_text_safe(bot, 1, 2, "hello", kb=None)
    bot.edit_message_text.assert_awaited_once_with(
        chat_id=1, message_id=2, text="hello", reply_markup=None, parse_mode="HTML"
    )
...
async def test_edit_message_text_safe_not_modified():
    bot = AsyncMock()
    bot.edit_message_text = AsyncMock(
        side_effect=TelegramBadRequest(method="editMessageText", message="Message is not modified")
    )
    await clear_edit_cache()
    await edit_message_text_safe(bot, 1, 2, "hello", kb=None)
...
async def test_edit_message_text_safe_parse_error_then_success():
    ...
    bot.edit_message_text = AsyncMock(
        side_effect=[
            TelegramBadRequest(
                method="editMessageText", message="can't parse entities: tag at byte offset 5"
            ),
            MagicMock(),
        ]
    )
    await clear_edit_cache()
    await edit_message_text_safe(bot, 1, 2, "<b>hello</b>", kb=None)
...
async def test_edit_message_text_safe_message_not_found():
    ...
    bot.edit_message_text = AsyncMock(
        side_effect=TelegramBadRequest(
            method="editMessageText", message="message to edit not found"
        )
    )
    await clear_edit_cache()
    result = await edit_message_text_safe(bot, 1, 2, "hello", kb=None)
...
async def test_edit_message_text_safe_cache():
    ...
    await edit_message_text_safe(bot, 1, 2, "hello", kb="kb1")
