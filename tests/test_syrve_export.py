...
def prepare_invoice_data(invoice):
    ...
    if not conception_id or not store_id:
        logger.warning("SYRVE_CONCEPTION_ID или SYRVE_STORE_ID не установлены, используем тестовые значения")
    ...
    for position in invoice.get("positions", []):
        ...
        items.append({
            ...
            "price": float(position.get("price", 0)) / 1000  # Конвертируем из миллиединиц в рубли
        })
...
async def main():
    ...
    try:
        ...
        logger.info(f"Подготовлены данные накладной: {json.dumps(invoice_data, indent=2, ensure_ascii=False)}")
