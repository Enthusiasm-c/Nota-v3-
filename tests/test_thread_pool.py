...
async def test_initialize_pool_creates_new():
    ...
    with patch("app.assistants.thread_pool.cache_get", return_value=None), patch(
        "app.assistants.thread_pool.cache_set"
    ) as mock_set, patch(
        "app.assistants.thread_pool.create_thread", new=AsyncMock(return_value="tid1")
    ):
...
async def test_initialize_pool_uses_existing():
    with patch("app.assistants.thread_pool.cache_get", return_value="tid1,tid2"), patch(
        "app.assistants.thread_pool.cache_set"
    ) as mock_set, patch(
        "app.assistants.thread_pool.create_thread", new=AsyncMock(return_value="tid3")
    ):
        client = MagicMock()
...
async def test_get_thread_from_pool():
    with patch("app.assistants.thread_pool.cache_get", return_value="tid1,tid2"), patch(
        ...
        "app.assistants.thread_pool.asyncio.create_task"
    ) as mock_task:
        client = MagicMock()
...
async def test_get_thread_creates_new_if_empty():
    with patch("app.assistants.thread_pool.cache_get", return_value=None), patch(
        "app.assistants.thread_pool.create_thread", new=AsyncMock(return_value="tidX")
    ):
        client = MagicMock()
        thread_id = await thread_pool.get_thread(client)
...
async def test_refill_pool_adds_threads():
    with patch("app.assistants.thread_pool.cache_get", return_value="tid1"), patch(
        "app.assistants.thread_pool.cache_set"
    ) as mock_set, patch(
        "app.assistants.thread_pool.create_thread", new=AsyncMock(side_effect=["tid2", "tid3"])
    ):
        client = MagicMock()
