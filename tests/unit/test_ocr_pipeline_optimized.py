...
def sample_cells():
    ...
    return [
        {
            'bbox': [10, 10, 100, 30],
            'image': create_test_image(90, 20, "Product A")
        },
        {
            'bbox': [110, 10, 150, 30],
            'image': create_test_image(40, 20, "5")
        },
        {
            'bbox': [160, 10, 200, 30],
            'image': create_test_image(40, 20, "pcs")
        },
        {
            'bbox': [210, 10, 250, 30],
            'image': create_test_image(40, 20, "100")
        },
        {
            'bbox': [260, 10, 300, 30],
            'image': create_test_image(40, 20, "500")
        },
        {
            'bbox': [10, 60, 100, 80],
            'image': create_test_image(90, 20, "Product B")
        },
        {
            'bbox': [110, 60, 150, 80],
            'image': create_test_image(40, 20, "2")
        },
        {
            'bbox': [160, 60, 200, 80],
            'image': create_test_image(40, 20, "kg")
        },
        {
            'bbox': [210, 60, 250, 80],
            'image': create_test_image(40, 20, "200")
        },
        {
            'bbox': [260, 60, 300, 80],
            'image': create_test_image(40, 20, "400")
        },
    ]
...
async def test_ocr_pipeline_init():
    ...
    with patch('app.ocr_pipeline_optimized.get_detector') as mock_get_detector, \
            patch('app.ocr_pipeline_optimized.PaddleOCR') as mock_paddle_ocr:
...
async def test_ocr_cell_with_gpt4o():
    ...
    with patch('app.ocr_pipeline_optimized.PaddleOCR') as mock_paddle_class, \
            patch('app.ocr_pipeline_optimized.process_cell_with_gpt4o') as mock_gpt4o:
        mock_paddle = MagicMock()
...
async def test_process_image_happy_path(sample_cells, mock_vision_result):
    ...
    with patch('app.ocr_pipeline_optimized.get_detector') as mock_get_detector, \
            patch('app.ocr_pipeline_optimized.OCRPipelineOptimized._process_cells') as mock_process_cells, \
            patch('app.ocr_pipeline_optimized.ValidationPipeline.validate') as mock_validate, \
            patch('app.ocr_pipeline_optimized.cache_get') as mock_cache_get, \
            patch('app.ocr_pipeline_optimized.cache_set') as mock_cache_set, \
            patch('app.ocr_pipeline_optimized.prepare_for_ocr') as mock_prepare:
...
async def test_process_image_table_detection_error():
    ...
    with patch('app.ocr_pipeline_optimized.get_detector') as mock_get_detector, \
            patch('app.ocr_pipeline_optimized.OCRPipelineOptimized._process_with_openai_vision') as mock_vision, \
            patch('app.ocr_pipeline_optimized.ValidationPipeline.validate') as mock_validate, \
            patch('app.ocr_pipeline_optimized.cache_get') as mock_cache_get:
