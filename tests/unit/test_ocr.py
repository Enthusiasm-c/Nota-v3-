...
from app.ocr import INVOICE_FUNCTION_SCHEMA, ParsedData, call_openai_ocr
...
def test_direct_vision_api(monkeypatch):
    ...
    json_data = {
        ...
        "date": "2025-01-01",
        "positions": [
            {"name": "Kacang", "qty": 1, "unit": "gr", "price": 10000, "total_price": 10000}
        ],
        "total_price": 10000,
    }
    ...
    monkeypatch.setattr(
        "app.ocr.base64",
        type("B", (), {"b64encode": lambda *a, **kw: b"xx", "decode": lambda *a, **kw: "xx"})(),
    )
...
def test_json_extraction_with_nested_data(monkeypatch):
    ...
    json_data = {
        ...
        "date": "2025-01-01",
        "positions": [
            {"name": "Product", "qty": 2, "unit": "kg", "price": 15000, "total_price": 30000}
        ],
        "total_price": 30000,
    }
    ...
    client = type(
        ...
        (),
        {
            "chat": type(
                "Chat",
                (),
                {"completions": type("Comp", (), {"create": lambda *a, **k: DummyResponse()})()},
            )()
        },
    )()
    monkeypatch.setattr("app.ocr.get_ocr_client", lambda: client)
...
def test_call_openai_ocr_cache_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.ocr.get_from_cache", lambda x: (_ for _ in ()).throw(Exception("cache error"))
    )
                                    ...
                                    type(
                                        ...
                                        (),
                                        {
                                            "function": type(
                                                "F",
                                                (),
                                                {"name": "get_parsed_invoice", "arguments": "{}"},
                                            )()
                                        },
                                    )
                                ]
...
def test_call_openai_ocr_image_opt_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.ocr.prepare_for_ocr", lambda *a, **k: (_ for _ in ()).throw(Exception("prep error"))
    )
...
def test_call_openai_cache_hit(monkeypatch):
    ...
    cached_result = ParsedData(
        supplier="Cached Supplier", date=date(2025, 1, 1), positions=[], total_price=1000
    )
...
def test_call_openai_ocr_api_timeout(monkeypatch):
    ...
    client.chat.completions.create.side_effect = openai.APITimeoutError(
        "Request timed out", request="dummy request"
    )
...
def test_call_openai_ocr_api_error(monkeypatch):
    ...
    client.chat.completions.create.side_effect = openai.APIError(
        "API error occurred", request="dummy request", http_status=500
    )
