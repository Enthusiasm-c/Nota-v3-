...
def test_normalize_product_name():
    ...
    assert normalize_product_name("apples") == "apple"  # Обработка множественного числа
    ...
    assert normalize_product_name("  TOMATOES  ") == "tomato"  # Обработка пробелов и регистра
...
def test_normalize_product_name_edge_cases():
    ...
    # Слово, которое не упоминается в словарях
    assert normalize_product_name("exotic_fruit") == "exotic_fruit"
    
    # Проверка, что нижний регистр применяется
    assert normalize_product_name("TOMATOES") == "tomato"
...
def test_calculate_string_similarity_product_variants():
    ...
    assert calculate_string_similarity("roma", "romaine") >= 0.8
    assert calculate_string_similarity("romaine lettuce", "romaine") >= 0.9
    assert calculate_string_similarity("tomato", "tomatoes") >= 0.9
    assert calculate_string_similarity("eggplant", "aubergine") >= 0.9
    assert calculate_string_similarity("chili", "chilli") >= 0.9
...
def test_fuzzy_find_threshold():
    ...
    res = fuzzy_find("app", products, thresh=0.9)
    assert len(res) == 0
    
    # С низким порогом находим частичные совпадения
    res = fuzzy_find("app", products, thresh=0.5)
    assert len(res) >= 1
...
def test_fuzzy_best_basic():
    ...
    assert best in catalog.keys()  # Возвращает что-то из каталога
    assert score < 50  # Низкий score
...
def test_match_supplier_object_suppliers():
    ...
    suppliers = [
        Supplier("1", "Apple Inc", "APPL"),
        Supplier("2", "Banana Corp", "BNNA")
    ]
...
def test_match_positions_basic():
    ...
    assert results[1]["status"] in ["ok", "partial"]
...
def test_match_positions_with_suggestions():
    ...
    positions = [
        {"name": "Appl", "qty": 1, "unit": "kg"}  # похоже на несколько продуктов
    ]
...
def test_match_positions_price_calculations():
    ...
    positions = [
        ...
        # Есть сумма, нет цены
        {"name": "Apple", "qty": 2, "total": 200},
        # Все значения заполнены
        {"name": "Apple", "qty": 2, "price": 100, "total": 200}
    ]
    
    results = match_positions(positions, products)
    
    # Проверяем вычисленные значения
    assert results[0]["line_total"] == 200  # 2 * 100
    assert results[1]["price"] == 100  # 200 / 2
    assert results[2]["line_total"] == 200
