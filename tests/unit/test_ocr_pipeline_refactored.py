...
def mock_paddle_ocr():
    ...
    paddle_mock.ocr.return_value = [[((0, 0, 100, 30), ("Test", 0.98))]]
    return paddle_mock
...
def mock_table_detector():
    ...
    detector_mock.extract_cells.return_value = [
        {"bbox": [0, 0, 100, 30], "image": b"test_image_data", "structure": {"text": "Product"}}
    ]
...
def sample_cells():
    ...
    return [
        ...
        {"bbox": [200, 0, 250, 30], "image": b"test_image_data", "structure": {"text": "100"}},
        {"bbox": [250, 0, 300, 30], "image": b"test_image_data", "structure": {"text": "100"}},
    ]
...
async def test_process_cells_empty():
    ...
    with patch("app.ocr_pipeline_refactored.get_detector") as mock_get_detector, patch(
        "app.ocr_pipeline_refactored.PaddleOCR"
    ) as mock_paddle_ocr, patch(
        "app.ocr_pipeline_refactored.ValidationPipeline"
    ) as mock_validation:
...
async def test_process_cells_all_errors(sample_cells):
    ...
    with patch("app.ocr_pipeline_refactored.PaddleOCR") as mock_paddle_ocr, patch.object(
        OCRPipeline, "process_cell_with_gpt4o", side_effect=Exception("GPT-4o Error")
    ):
...
async def test_process_with_openai_vision_success():
    ...
    with patch(
        "app.ocr_pipeline_refactored.call_openai_ocr_async", new_callable=AsyncMock
    ) as mock_call_ocr, patch("app.ocr_pipeline_refactored.settings") as mock_settings:
...
def test_send_to_gpt_normal():
    ...
    with patch("openai.chat.completions.create") as mock_create, patch(
        "app.ocr_pipeline_refactored.settings"
    ) as mock_settings:
...
def test_send_to_gpt_error():
    ...
    with patch("openai.chat.completions.create", side_effect=Exception("API Error")), patch(
        "app.ocr_pipeline_refactored.settings"
    ) as mock_settings:
...
async def test_ocr_cell_success(sample_image_bytes):
    ...
    with patch("app.ocr_pipeline_refactored.PaddleOCR") as mock_paddle_ocr:
        # Configure mock
        paddle_instance = MagicMock()
        paddle_instance.ocr.return_value = [[((0, 0, 100, 30), ("Cell Text", 0.98))]]
        mock_paddle_ocr.return_value = paddle_instance
        ...
        cell = {"bbox": [0, 0, 100, 30], "image": sample_image_bytes, "structure": {"text": "Test"}}
...
async def test_ocr_cell_too_small():
    ...
    with patch("app.ocr_pipeline_refactored.PaddleOCR") as mock_paddle_ocr, patch(
        "PIL.Image.open"
    ) as mock_image_open:
...
async def test_build_lines_from_cells():
    ...
    cells = [
        {"bbox": [0, 10, 100, 40], "text": "Product A", "confidence": 0.95, "used_gpt4o": False},
        {"bbox": [100, 12, 150, 42], "text": "10", "confidence": 0.98, "used_gpt4o": False},
        {"bbox": [150, 11, 200, 41], "text": "kg", "confidence": 0.97, "used_gpt4o": False},
        {"bbox": [200, 9, 250, 39], "text": "1000", "confidence": 0.99, "used_gpt4o": False},
        {"bbox": [250, 10, 300, 40], "text": "10000", "confidence": 0.99, "used_gpt4o": False},
    ]
...
async def test_numeric_parsing_edge_cases():
    ...
    assert (
        OCRPipeline.parse_numeric_value("10.000,00", default=0, is_float=True) == 10000.00
    )  # European style
    assert (
        OCRPipeline.parse_numeric_value("10,000.00", default=0, is_float=True) == 10000.00
    )  # US style
