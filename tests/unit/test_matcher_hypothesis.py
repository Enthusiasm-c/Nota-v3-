...
product_names = st.one_of(
    ...
    st.lists(st.text(min_size=1, max_size=10), min_size=1, max_size=5).map(
        " ".join
    ),  # Multi-word strings
    st.sampled_from(
        [
            "a",
            "b",
            "c",  # Single letter words
            "an",
            "to",
            "of",  # Common short words
            "a a",
            "a b",
            "c d",  # Combinations of short words
...
line_item_dict = st.fixed_dictionaries(
    {
        "name": product_names,
        "qty": st.one_of(
            st.integers(min_value=1, max_value=100), st.floats(min_value=0.1, max_value=100.0)
        ),
...
@given(
    s1=st.text(min_size=1, max_size=20, alphabet="abcdefghijklmnopqrstuvwxyz"),
...
@given(query=product_names, products=st.lists(product_dict, min_size=1, max_size=30))
@settings(max_examples=50)
...
@given(
    supplier_name=st.text(min_size=1, max_size=30),
...
@given(
    positions=st.lists(line_item_dict, min_size=1, max_size=5),
...
def test_match_positions_properties(positions, products, threshold):
    ...
    results_with_suggestions = match_positions(
        positions, products, threshold, return_suggestions=True
    )
...
@given(
    s1=st.text(
        min_size=1, max_size=1, alphabet=st.characters(blacklist_categories=("Cs",))
    ),  # Single character strings
    s2=st.text(min_size=1, max_size=1, alphabet=st.characters(blacklist_categories=("Cs",))),
)  # Single character strings
@settings(max_examples=30)
