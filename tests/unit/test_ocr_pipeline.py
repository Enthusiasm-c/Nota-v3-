...
from app.ocr_helpers import build_lines_from_cells, parse_numeric_value, process_cell_with_gpt4o
...
async def test_process_cells_with_ocr_errors(sample_cells):
    ...
    with patch("app.ocr_pipeline.PaddleOCR") as mock_paddle_ocr, patch(
        "app.ocr_helpers.process_cell_with_gpt4o", new_callable=AsyncMock
    ) as mock_gpt4o, patch("app.ocr_helpers.prepare_cell_image") as mock_prepare:
...
async def test_process_cells_row_grouping(sample_cells):
    ...
    with patch("app.ocr_pipeline.PaddleOCR") as mock_paddle_ocr, patch(
        "app.ocr_helpers.prepare_cell_image"
    ) as mock_prepare:
...
async def test_process_with_openai_vision_success():
    ...
    with patch(
        "app.ocr_pipeline.call_openai_ocr_async", new_callable=AsyncMock
    ) as mock_call_ocr, patch("app.ocr_pipeline.settings") as mock_settings:
...
def test_send_to_gpt_normal():
    ...
    with patch("app.ocr_pipeline.openai") as mock_openai, patch(
        "app.ocr_pipeline.settings"
    ) as mock_settings:
...
async def test_ocr_cell_too_small():
    ...
    with patch("app.ocr_pipeline.PaddleOCR") as mock_paddle_ocr, patch(
        ...
        cell = {"bbox": [0, 0, 5, 5], "image": b"tiny_image", "structure": {"text": "Test"}}
        result = await pipeline._ocr_cell(cell)
...
async def test_empty_cells_handling():
    ...
    with patch("app.ocr_pipeline.PaddleOCR") as mock_paddle_ocr, patch(
        ...
        # Create sample cells
        cells = [{"bbox": [0, 0, 100, 30], "image": b"test_image_data", "structure": {"text": ""}}]
...
async def test_all_empty_cells_handling():
    ...
    with patch("app.ocr_pipeline.PaddleOCR") as mock_paddle_ocr, patch(
        ...
        # Configure mocks
        paddle_instance = MagicMock()
        paddle_instance.ocr.return_value = [[((0, 0, 100, 30), ("", 0.1))]]  # Empty text result
        mock_paddle_ocr.return_value = paddle_instance
...
async def test_paddle_ocr_import_error():
    ...
    with patch(
        "app.ocr_pipeline.PaddleOCR", side_effect=ImportError("No module named 'paddleocr'")
    ):
