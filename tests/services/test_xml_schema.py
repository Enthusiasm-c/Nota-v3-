...
from app.services.syrve_invoice_sender import Invoice, InvoiceItem, SyrveClient
...
class XMLSchemaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.invoice_minimal = Invoice(
            items=[self.item], supplier_id=self.supplier_id, default_store_id=self.store_id
        )
    ...
    def test_xml_order(self):
        ...
        self.assertLess(
            default_store_pos, conception_pos, "defaultStore должен быть перед conception"
        )
        self.assertLess(
            conception_pos, document_number_pos, "conception должен быть перед documentNumber"
        )
        self.assertLess(
            document_number_pos, date_incoming_pos, "documentNumber должен быть перед dateIncoming"
        )
        self.assertLess(
            date_incoming_pos, external_id_pos, "dateIncoming должен быть перед externalId"
        )
