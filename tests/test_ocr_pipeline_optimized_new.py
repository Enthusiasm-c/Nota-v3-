...
class TestOCRPipelineOptimized:
    ...
    async def test_process_image_cache_hit(
        self, mock_prepare, mock_cache_get, pipeline, mock_image_bytes
    ):
    ...
    async def test_process_image_success_path(
        self,
        mock_get_detector,
        mock_prepare,
        mock_cache_set,
        ...
        expected_lines = [{"name": "Test Item", "qty": 1, "price": 100}]
    ...
    @patch("app.ocr_pipeline_optimized.prepare_for_ocr")
    @patch("app.ocr_pipeline_optimized.get_detector")
    async def test_process_image_table_detection_error_with_fallback(
        self, mock_get_detector, mock_prepare, mock_cache_get, pipeline, mock_image_bytes
    ):
    ...
    @patch("app.ocr_pipeline_optimized.prepare_for_ocr")
    @patch("app.ocr_pipeline_optimized.get_detector")
    async def test_process_image_table_detection_error_no_fallback(
        self, mock_get_detector, mock_prepare, mock_cache_get, mock_image_bytes
    ):
    ...
    @patch("app.ocr_pipeline_optimized.cache_get")
    @patch("app.ocr_pipeline_optimized.prepare_for_ocr")
    async def test_process_image_optimization_error_non_critical(
        self, mock_prepare, mock_cache_get, pipeline, mock_image_bytes
    ):
        ...
        with patch("app.ocr_pipeline_optimized.get_detector") as mock_get_detector:
            ...
            pipeline.validation_pipeline.validate.return_value = {"status": "success", "lines": []}
    ...
    async def test_process_with_openai_vision_success(
        self, mock_openai_ocr, pipeline, mock_image_bytes
    ):
        """Тест успешной обработки через OpenAI Vision"""
        # Мокируем ответ OpenAI
        openai_response = json.dumps(
            {"lines": [{"name": "Apple", "qty": 5, "unit": "kg", "price": 100, "amount": 500}]}
        )
    ...
    @pytest.mark.asyncio
    @patch("app.ocr_pipeline_optimized.call_openai_ocr_async")
    async def test_process_with_openai_vision_positions_format(
        self, mock_openai_ocr, pipeline, mock_image_bytes
    ):
    ...
    @pytest.mark.asyncio
    @patch("app.ocr_pipeline_optimized.call_openai_ocr_async")
    async def test_process_with_openai_vision_retry_logic(
        self, mock_openai_ocr, pipeline, mock_image_bytes
    ):
        """Тест логики повторных попыток в OpenAI Vision"""
        # Первый вызов неудачный, второй успешный
        mock_openai_ocr.side_effect = [Exception("API Error"), json.dumps({"lines": []})]
    ...
    @pytest.mark.asyncio
    @patch("app.ocr_pipeline_optimized.call_openai_ocr_async")
    async def test_process_with_openai_vision_all_retries_fail(
        self, mock_openai_ocr, pipeline, mock_image_bytes
    ):
    ...
    async def test_ocr_cell_small_uses_gpt4o(self, mock_gpt4o, mock_prepare_cell, pipeline):
        ...
        mock_prepare_cell.return_value = np.zeros(
    ...
    @pytest.mark.asyncio
    @patch("app.ocr_pipeline_optimized.build_lines_from_cells")
    async def test_process_cells_parallel_processing(
        self, mock_build_lines, pipeline, sample_cells
    ):
    ...
    @pytest.mark.asyncio
    @patch("app.ocr_pipeline_optimized.build_lines_from_cells")
    async def test_process_cells_gpt4o_usage_tracking(
        self, mock_build_lines, pipeline, sample_cells
    ):
    ...
    async def test_process_cells_parallel_error_fallback(self, mock_build_lines, pipeline):
        ...
        problem_cells = [
            {"image": np.zeros((20, 20, 3), dtype=np.uint8), "problem": True} for _ in range(5)
        ]
...
class TestErrorHandling:
    ...
    @patch("app.ocr_pipeline_optimized.cache_get")
    @patch("app.ocr_pipeline_optimized.prepare_for_ocr")
    async def test_general_processing_error(
        self, mock_prepare, mock_cache_get, pipeline, mock_image_bytes
    ):
...
class TestCacheIntegration:
    ...
    async def test_cache_error_handling(
        self, mock_cache_set, mock_cache_get, pipeline, mock_image_bytes
    ):
