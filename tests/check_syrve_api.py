...
async def main():
    ...
    try:
        ...
        await check_endpoint(syrve_client, auth_token, "settings/restaurantSettings", "Настройки ресторана")
        
        # Проверка эндпоинта для получения накладных (не для импорта)
        date_range = "from=2025-01-01&to=2025-05-11"
        await check_endpoint(syrve_client, auth_token, f"documents/export/incomingInvoice?{date_range}", "Экспорт накладных")
