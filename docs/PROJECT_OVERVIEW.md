...
**Обновление 2025-05-03:**
- Реализовано строгое сопоставление алиасов: теперь alias сравнивается с позицией без учета регистра, без лишней логики fuzzy fallback для алиасов.
- После добавления нового алиаса через inline-редактирование или тесты, он становится доступен для сопоставления сразу после следующей загрузки продуктов (load_products).
- Кэширование продуктов и алиасов (через @lru_cache) отключено — всегда актуальные данные при любом изменении файлов.
- Тесты alias flow полностью проходят: добавленный alias мгновенно распознается.
- Рекомендация: если Source Control не видит изменений, убедитесь, что все файлы сохранены, а git status показывает ожидаемые изменения.
- Все изменения в alias-логике и тестах теперь отражены в этом файле и в коде.
...
| Module | State | Notes |
|--------|-------|-------|
| Skeleton bot (aiogram 3) | Done | Fixed critical issues with imports and message handling |
...
| Formatter (Markdown report RU/EN) | Done | Fixed Markdown escaping issues for stable rendering |
| Data 🤖 base_suppliers.csv, base_products.csv | Done | |
| aliases.csv (self-learning) | Done | Growing with user corrections |
| Message Handling | Done | New approach using message creation instead of editing |
| Field Editing | Done | Direct field updates without assistant API dependencies |
| Logging & Debugging | Done | Optimized log levels and diagnostics capabilities |
| CI (GitHub Actions) + tests | Done | Including tests for API error handling |
| Docs (this file + API error handling) | Done | Updated with critical bugs and solutions |
| Deploy to DO / systemd | Done | Successfully deployed and operational |	
...
16 • Критические ошибки и их решения (Обновлено 2025-05-04)

В процессе эксплуатации бота на сервере было обнаружено и исправлено несколько критических ошибок:

### 16.1 Ошибки импорта и синтаксические ошибки

**Проблема:** Бот не запускался на сервере из-за ошибок:
- `TelegramBadRequest` импортировался из неправильного модуля (`aiogram.types` вместо `aiogram.exceptions`)
- Синтаксические ошибки в строковых литералах в файлах `bot.py` и `md.py`

**Решение:**
- Исправлен импорт с `from aiogram.types import TelegramBadRequest` на `from aiogram.exceptions import TelegramBadRequest`
- Исправлены синтаксические ошибки в строковых литералах с использованием raw-строк

### 16.2 Проблемы форматирования Markdown

**Проблема:** Сообщения бота с отчетами кратковременно появлялись и исчезали, в логах было множество ошибок "can't parse entities".

**Решение:**
- Полностью переработана функция `escape_v2` в `app/utils/md.py` для более надежного экранирования специальных символов
- Добавлена особая обработка блоков кода (сохранение текста между маркерами ```без экранирования)
- Реализована защита от двойного экранирования специальных символов
- Добавлена дополнительная проверка на неэкранированные символы, особенно точки, решетки и т.д.

### 16.3 Нестабильное отображение сообщений

**Проблема:** Даже после исправления экранирования, сообщения все равно иногда исчезали. Telegram API возвращал ошибки при попытке редактирования сообщений.

**Решение:**
- Полностью отказались от редактирования сообщений в пользу отправки новых
- Изменили подход к обработке состояний диалога, добавив маркер `editing_mode` для отслеживания контекста
- Реализовали обновление ссылок на сообщения в `user_matches` при создании новых сообщений
- Добавили поиск по ID пользователя как запасной вариант при потере связи с конкретным сообщением

### 16.4 Проблема с ответами от OpenAI Assistant

**Проблема:** При попытке редактирования полей инвойса пользователь получал ответ "I'm sorry, I can discuss Nota AI–related topics only" вместо обновленного отчета.

**Решение:**
- Переписали обработчик текстовых сообщений для правильной маршрутизации в зависимости от текущего режима
- Заменили вызовы OpenAI Assistant при редактировании полей на прямое обновление данных
- Реализовали механизм очистки режима редактирования после обработки изменений

### 16.5 Логирование и отладка

**Проблема:** В логах присутствовало много "мусорного кода", затрудняющего отладку.

**Решение:**
- Оптимизированы уровни логирования для разных модулей
- Уменьшена детализация для сторонних библиотек (httpx, aiogram и т.д.)
- Добавлена возможность временного повышения уровня логирования для отладки
- Стандартизировано форматирование логов с контекстной информацией (ID запроса, ID пользователя)

Все эти изменения значительно повысили стабильность работы бота и улучшили пользовательский опыт, особенно при работе с большими отчетами и при редактировании полей инвойса.
