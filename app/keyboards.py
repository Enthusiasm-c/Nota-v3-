...
def kb_report(match_results: list) -> InlineKeyboardMarkup:
    ...
    edit_buttons = [
        [
    InlineKeyboardButton(
        text=f"✏️ Edit line {i+1}", callback_data=f"edit:{i}"
    )
]
...
def build_invoice_report(match_results: list, page: int = 1, total_pages: int = 1) -> InlineKeyboardMarkup:
    ...
    add_missing_btn = InlineKeyboardButton(
        text="➕ Add missing",
        callback_data="inv_add_missing"
    )
    # Кнопка Submit anyway
    submit_anyway_btn = InlineKeyboardButton(
        text="✅ Submit anyway",
        callback_data="inv_submit_anyway"
    )
    # Кнопка Back
    back_btn = InlineKeyboardButton(
        text="↩ Back",
        callback_data="inv_cancel_edit"
    )
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Name",
                    callback_data=f"field:name{suffix}"
                ),
                InlineKeyboardButton(
                    text="Qty",
                    callback_data=f"field:qty{suffix}"
                ),
                InlineKeyboardButton(
                    text="Unit",
                    callback_data=f"field:unit{suffix}"
                ),
                InlineKeyboardButton(
                    text="Price",
                    callback_data=f"field:price{suffix}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Cancel",
                    callback_data=f"cancel{suffix}"
                )
            ]
        ]
    )
...
def kb_cancel_all() -> InlineKeyboardMarkup:
    ...
    text="🚫 Cancel", callback_data="cancel:all"
)]]
    )
