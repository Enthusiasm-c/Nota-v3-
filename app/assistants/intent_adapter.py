...
class IntentAdapter:
    ...
    FAST_PATTERNS = {
        ...
        r"(?:строк[аи]?|line|row)\s+(\d+).*?(?:цен[аыу]|price)\s+(\d+)": lambda m: {
            "action": "set_price",
            "line_index": int(m.group(1)) - 1,
            "value": m.group(2),
        },
        # Количество: строка 2 количество 5
        r"(?:строк[аи]?|line|row)\s+(\d+).*?(?:кол-во|количество|qty|quantity)\s+(\d+)": lambda m: {
            "action": "set_quantity",
            "line_index": int(m.group(1)) - 1,
            "value": m.group(2),
        },
        # Единица измерения: строка 3 ед изм кг
        r"(?:строк[аи]?|line|row)\s+(\d+).*?(?:ед\.?\s*изм\.?|unit)\s+(\w+)": lambda m: {
            "action": "set_unit",
            "line_index": int(m.group(1)) - 1,
            "value": m.group(2),
        },
        # Название: строка 1 название Apple
        r"(?:строк[аи]?|line|row)\s+(\d+).*?(?:имя|название|name)\s+(.+?)(?:$|\s+(?:цен|кол|ед))": lambda m: {
            "action": "set_name",
            "line_index": int(m.group(1)) - 1,
            "value": m.group(2).strip(),
        },
        # Дата: дата 15 мая
        r"дат[аы]?\s+(\d{1,2})\s+(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)": lambda m: {
            "action": "set_date",
            "value": f"{datetime.now().year}-{IntentAdapter.MONTHS.get(m.group(2), 1):02d}-{int(m.group(1)):02d}",
        },
        # Дата числовая: дата 15.05.2023
        r"дат[аы]?\s+(\d{1,2})[./](\d{1,2})(?:[./](\d{4}))?": lambda m: {
            "action": "set_date",
            "value": f"{m.group(3) or datetime.now().year}-{int(m.group(2)):02d}-{int(m.group(1)):02d}",
        },
    }
    ...
    def adapt(cls, response: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(response, str):
                ...
                if fast_result:
                    logger.info(
                        f"[IntentAdapter] Быстрое распознавание команды: {fast_result.get('action')}"
                    )
                    return fast_result
                ...
                if cached_intent:
                    try:
                        ...
                        logger.info(
                            f"[IntentAdapter] Использую кешированную команду: {intent.get('action')}"
                        )
                        return intent
                    except Exception as cache_err:
                        logger.warning(
                            f"[IntentAdapter] Ошибка при разборе кешированной команды: {cache_err}"
                        )
            ...
            if (
                "actions" in intent
                and isinstance(intent["actions"], list)
                and len(intent["actions"]) > 0
            ):
                logger.info(
                    f"Найден массив actions, извлекаем первое действие: {intent['actions'][0]}"
                )
                ...
                if isinstance(action_item, dict) and "action" in action_item:
                    ...
                    logger.error(
                        f"Элемент массива actions не содержит поле 'action': {action_item}"
                    )
            ...
            if missing_fields:
                logger.error(
                    f"Отсутствуют обязательные поля {missing_fields} для действия {action}"
                )
                return {
                    "action": "unknown",
                ...
                try:
                    ...
                    if isinstance(parsed_json, dict):
                        if "actions" in parsed_json or "action" in parsed_json:
                            ...
                            if logger.isEnabledFor(logging.DEBUG):
                                logger.debug(
                                    f"Извлеченный JSON не содержит поля actions/action: {parsed_json}"
                                )
                    return parsed_json
                except json.JSONDecodeError:
                    logger.warning(
                        f"Не удалось распарсить основной JSON фрагмент: {json_str[:100]}..."
                    )
                    ...
                    for s, e, _ in fragments:
                        try:
                            ...
                            if isinstance(parsed_json, dict):
                                if "actions" in parsed_json or "action" in parsed_json:
                                    logger.info(
                                        "Successfully extracted alternative JSON with actions/action field"
                                    )
                                    if logger.isEnabledFor(logging.DEBUG):
    ...
    def _parse_text_intent(cls, text: str) -> Dict[str, Any]:
        ...
        if "дат" in text_lower or "date" in text_lower:
            ...
            # Ищем день (число)
            day_match = re.search(
                r"(\d{1,2})(?:\s+|[-./])(?:января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря|january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)",
                text_lower,
            )
            if day_match:
        ...
        price_match = re.search(
            r"(строк[аи]?|line|row)\s+(\d+).*?(цен[аы]|price)\s+(\d+)", text_lower
        )
        if price_match:
            try:
        ...
        name_match = re.search(
            r"(строк[аи]?|line|row)\s+(\d+).*?(имя|название|name)\s+(.+?)(?:\s*$|\s+\w+\s+)",
            text_lower,
        )
        if name_match:
            try:
        ...
        try:
            ...
            if "line" in intent and "line_index" not in intent:
                ...
                try:
                    ...
                    logger.info(
                        f"Преобразовано поле 'row' в 'line_index': {normalized['line_index']}"
                    )
                except (ValueError, TypeError):
                    normalized["line_index"] = 0  # Значение по умолчанию
