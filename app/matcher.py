...
def fuzzy_match_product(query: str, products: List[Union[Product, Dict[str, Any]]], threshold: float = 0.7) -> Tuple[Optional[Union[Product, Dict[str, Any]]], float]:
    """
...
def find_best_match(position: Dict[str, Any], products: List[Union[Product, Dict[str, Any]]], threshold: float = 0.7) -> Dict[str, Any]:
    """
    ...
    if position_unit:
        if isinstance(matched_product, dict):
            ...
            product_unit = getattr(matched_product, "unit", "") if hasattr(matched_product, "unit") else ""
    ...
    if position_price:
        if isinstance(matched_product, dict):
            ...
            product_price_hint = getattr(matched_product, "price_hint", None) if hasattr(matched_product, "price_hint") else None
