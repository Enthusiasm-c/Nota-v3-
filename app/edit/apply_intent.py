...
def set_name(
    invoice: Dict[str, Any], line_index: int, value: str, manual_edit: bool = False
) -> Dict[str, Any]:
    """
...
def set_unit(invoice: Dict[str, Any], line_index: int, value: str) -> Dict[str, Any]:
    ...
    if 0 <= line_index < len(result.get("positions", [])):
        ...
        logger.warning(
            f"Попытка установить единицу измерения для несуществующей строки: {line_index + 1}"
        )
...
def apply_intent(invoice: Union[dict, ParsedData], intent: dict) -> dict:
    ...
    if action == "set_date":
        ...
        return set_price(invoice, intent.get("line_index", 0), intent.get("value", ""))

    elif action == "set_name":
        return set_name(invoice, intent.get("line_index", 0), intent.get("value", ""))
        ...
        if isinstance(invoice.get("positions"), list) and 0 <= line < len(invoice["positions"]):
            ...
            if field in invoice["positions"][line]:
                ...
                logger.info(
                    f"Line {line+1} field '{field}' manually edited by user: value = '{value}'"
                )
