...
import time
import uuid
from typing import Callable, TypeVar, Optional, Any, Union, Dict, Tuple
...
def classify_error(error: Exception) -> Tuple[str, str]:
    ...
    return (ErrorType.UNKNOWN, 
            f"Неизвестная ошибка. Пожалуйста, попробуйте еще раз.")
...
def with_retry_backoff(
    ...
    """
    Декоратор для обычных (не async) функций, добавляющий повторные попытки с экспоненциальной задержкой.
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                error_stage = next((stage for stage, done in stages_state.items() 
                                   if not done), None)
                ...
                if error_stage:
                    ...
                    logger.error(
                        f"[{req_id}] {error_class} error in {func.__name__} at stage '{stage_name}': {str(e)}"
                    )
