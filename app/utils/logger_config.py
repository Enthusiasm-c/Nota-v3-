...
def configure_logging(environment="development", log_dir="logs"):
    ...
    bot_handler = logging.handlers.RotatingFileHandler(
        f"{log_dir}/bot.log", maxBytes=10 * 1024 * 1024, backupCount=5  # 10 MB
    )
    ...
    file_handler = logging.handlers.RotatingFileHandler(
        f"{log_dir}/nota.log", maxBytes=10 * 1024 * 1024, backupCount=5  # 10 MB
    )
    ...
    error_handler = logging.handlers.RotatingFileHandler(
        f"{log_dir}/errors.log", maxBytes=5 * 1024 * 1024, backupCount=3  # 5 MB
    )
    ...
    logging.getLogger("app").info(
        f"Logging configured for {environment} environment with {len(root_logger.handlers)} handlers"
    )
