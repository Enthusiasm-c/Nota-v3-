            ...
            while not _CLEANUP_THREAD_STOP.is_set():
                ...
                with _STRING_CACHE_LOCK:
                    ...
                    if _STRING_CACHE_HITS + _STRING_CACHE_MISSES > 10000:
                        hit_rate = (
                            _STRING_CACHE_HITS / (_STRING_CACHE_HITS + _STRING_CACHE_MISSES) * 100
                        )
                        logger.info(
                            f"Статистика кеша строк: {_STRING_CACHE_HITS} попаданий, "
...
def cache_data(cache_key: str, ttl: int = 3600) -> Callable:
    """
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            # Cache result
            _DATA_CACHE[cache_key] = (result, current_time)
...
def cached_load_data(
    path: str, loader_func: Callable, cache_key: str = None
) -> List[Dict[str, Any]]:
    """
    ...
    with _CACHE_LOCK:
        ...
        if (
            full_key in _DATA_CACHE
            and full_key in _MODIFIED_TIMES
            and current_mtime <= _MODIFIED_TIMES[full_key]["mtime"]
            and last_check_time - _MODIFIED_TIMES[full_key]["checked"] < CHECK_INTERVAL
        ):
            logger.debug(
                f"Используем кешированные данные для {path} ({len(_DATA_CACHE[full_key])} записей)"
            )
            return _DATA_CACHE[full_key]
        ...
        try:
            ...
            _MODIFIED_TIMES[full_key] = {"mtime": current_mtime, "checked": last_check_time}
...
async def cached_load_data_async(
    path: str, loader_func: Callable, cache_key: str = None
) -> List[Dict[str, Any]]:
    """
    ...
    if not os.path.exists(path):
        ...
        with _CACHE_LOCK:
            if full_key in _DATA_CACHE:
                logger.warning(
                    f"Используем кешированные данные для недоступного файла {path} (async)"
                )
                return _DATA_CACHE[full_key]
        return []
