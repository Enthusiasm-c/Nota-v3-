...
async def process_image_step(step_func, *args, **kwargs):
    ...
    """
    return await asyncio.wait_for(asyncio.to_thread(step_func, *args, **kwargs), timeout=90.0)


async def process_invoice_pipeline(
    img_bytes: bytes, tmp_path: str, req_id: str
) -> Tuple[bytes, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"[{req_id}] Invoice metadata: supplier='{supplier}', date='{date}', number='{invoice_num}'"
        )
