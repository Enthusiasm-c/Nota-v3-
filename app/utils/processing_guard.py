...
async def set_user_busy(user_id: int, context_name: str = "default", max_age: int = 120) -> None:
    ...
    _active_users[user_id][context_name] = {"timestamp": time.time(), "max_age": max_age}
...
async def _auto_clear_edit(user_id: int) -> None:
    ...
    if user_id in _processing_edit:
        ...
        logger.debug(
            f"Автоочистка блокировки редактирования для user_id={user_id}, prev_state={prev_state}"
        )
...
async def check_rate_limit(
    user_id: int, action_type: str, min_interval: float = 2.0, notify: bool = False
) -> bool:
    """
    ...
    if action_type in _user_request_timestamps[user_id]:
        ...
        if elapsed < min_interval:
            if notify:
                logger.warning(
                    f"Rate limit for user {user_id} on {action_type}: {elapsed:.2f}s < {min_interval}s"
                )
            return False
...
def clear_all_locks():
    ...
    logger.info(
        f"All user locks and flags cleared: active_users={len(_active_users)}, photo_users={len(_processing_photo_users)}, syrve_users={len(_sending_to_syrve_users)}, request_timestamps={len(_user_request_timestamps)}"
    )
    print(
        f"[GUARD] Cleared all locks: active_users={len(_active_users)}, photo_users={len(_processing_photo_users)}, syrve_users={len(_sending_to_syrve_users)}"
    )
