...
class SyrveClient:
    ...
    def validate_invoice(self, invoice: Invoice) -> None:
        ...
        if not GUID_REGEX.match(invoice.default_store_id):
            raise InvoiceValidationError(f"Invalid default_store_id format: {invoice.default_store_id}")
        ...
        for item in invoice.items:
            ...
            expected_sum = (item.amount * item.price).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if is_valid is not None and is_valid.text.lower() == "true":
                        logger.info(
                            "Invoice successfully imported to Syrve (%.2f seconds)", 
                            elapsed_time
                        )
                        if self.on_result:
                ...
                if status_code in (502, 503, 504) and retries < self.max_retries:
                    ...
                    logger.warning(
                        ...
                        status_code, retries, self.max_retries, sleep_time
                    )
                ...
                # Логируем ошибку
                logger.error(
                    "Syrve API HTTP %d error: %s", 
                    status_code, e.response.text
                )
                ...
                if retries < self.max_retries:
                    ...
                    logger.warning(
                        ...
                        retries, self.max_retries, sleep_time, str(e)
                    )
                    time.sleep(sleep_time)
                    last_error = e
                    continue
