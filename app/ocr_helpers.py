...
def parse_numeric_value(text: Optional[str], default: float = 0, is_float: bool = False) -> float:
    ...
    try:
        ...
        logger.warning(f"Failed to convert '{original_text}' to number: {e}. Using default: {default}")
        return default
...
async def process_cell_with_gpt4o(cell_image_bytes: bytes) -> Tuple[str, float]:
    ...
    cell_prompt = "Look carefully at this image. It contains text from a single table cell. " \
            ...
            lines = [
                {
                    ...
                    'price': 100,
                    'amount': 500,
                    'cells': [{'text': c.get('text', ''), 'confidence': c.get('confidence', 0), 
                              'used_gpt4o': c.get('used_gpt4o', False)} for c in row1_cells]
                },
                {
                    'name': 'Product B',
                    'qty': 2,
                    ...
                    'amount': 400,
                    'cells': [{'text': c.get('text', ''), 'confidence': c.get('confidence', 0),
                              'used_gpt4o': c.get('used_gpt4o', False)} for c in row2_cells]
                }
            ]
            return lines
