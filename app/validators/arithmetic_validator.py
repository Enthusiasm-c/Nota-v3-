...
class ArithmeticValidator:
    ...
    def validate(self, invoice_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, line in enumerate(lines):
            ...
            try:
                ...
                if self._is_missing_or_zero(amount) and not self._is_missing_or_zero(qty) and not self._is_missing_or_zero(price):
                ...
                elif self._is_missing_or_zero(qty) and not self._is_missing_or_zero(amount) and not self._is_missing_or_zero(price):
                ...
                elif self._is_missing_or_zero(price) and not self._is_missing_or_zero(amount) and not self._is_missing_or_zero(qty):
                    ...
                    if not self._is_missing_or_zero(qty) and not self._is_missing_or_zero(price) and not self._is_missing_or_zero(amount):
    ...
    def _parse_numeric(self, value: Any) -> float:
        ...
        if isinstance(value, (int, float)):
            ...
            try:
                ...
                logger.warning(f"Не удалось преобразовать '{value}' в число. Использую значение по умолчанию: 0")
                return 0.0
        else:
            return 0.0
    ...
    def _try_fix_decimal_errors(self, qty: float, price: float, amount: float) -> Optional[Dict[str, float]]:
        """
    ...
    def _try_fix_zero_errors(self, qty: float, price: float, amount: float) -> Optional[Dict[str, float]]:
        """
