...
def generate_report(before_stats, after_stats, output_file):
    ...
    if "ocr_time" in improvements:
        print(f"Среднее время OCR: {improvements['ocr_time']['before']:.2f}s → {improvements['ocr_time']['after']:.2f}s")
...
def main():
    ...
    parser.add_argument("--before", required=True, help="Директория с результатами тестов до оптимизаций")
    parser.add_argument("--after", required=True, help="Директория с результатами тестов после оптимизаций")
    parser.add_argument("--output", default="optimization_report.json", help="Путь для сохранения отчета")
